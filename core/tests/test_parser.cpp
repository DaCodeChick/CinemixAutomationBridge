...
void onMalformed(void* user) { static_cast<void>(user); }
...
TEST_CASE("parser: single CC message") {
    ...
    CHECK_EQ(c.channels.size(), static_cast<size_t>(1));
    CHECK_EQ(static_cast<int>(c.channels[0]), 1);
    CHECK_EQ(static_cast<int>(c.ccs[0]), 10);
    CHECK_EQ(static_cast<int>(c.values[0]), 20);
}

TEST_CASE("parser: running status") {
    Captured c = parse({0xB0, 10, 20, 30, 40});
    CHECK_EQ(c.channels.size(), static_cast<size_t>(2));
    CHECK_EQ(static_cast<int>(c.ccs[0]), 10);
    CHECK_EQ(static_cast<int>(c.values[0]), 20);
    CHECK_EQ(static_cast<int>(c.ccs[1]), 30);
    CHECK_EQ(static_cast<int>(c.values[1]), 40);
}

TEST_CASE("parser: running status switches on new status byte") {
    Captured c = parse({0xB0, 10, 20, 0xB1, 5, 6});
    CHECK_EQ(c.channels.size(), static_cast<size_t>(2));
    CHECK_EQ(static_cast<int>(c.channels[1]), 2);
    CHECK_EQ(static_cast<int>(c.ccs[1]), 5);
}
...
TEST_CASE("parser: realtime bytes interleaved inside a message") {
    ...
    CHECK_EQ(c.systemBytes.size(), static_cast<size_t>(0));
}

TEST_CASE("parser: system reset byte reported") {
    Captured c = parse({0xFF});
    CHECK_EQ(c.systemBytes.size(), static_cast<size_t>(1));
    CHECK_EQ(static_cast<int>(c.systemBytes[0]), 0xFF);
}
...
TEST_CASE("parser: stray data bytes count as malformed") {
    ...
    Captured c;
    p.setHandlers(&c, &onCc, &onSystem, &onMalformed);
    const std::uint8_t stray[] = {0x40, 0x41, 0x42};
    p.feed(stray, sizeof(stray));
    CHECK_EQ(c.channels.size(), static_cast<size_t>(0));
...
TEST_CASE("parser: byte-at-a-time feed equals bulk feed") {
    ...
    for (size_t i = 0; i < bulk.ccs.size(); ++i) {
        CHECK_EQ(static_cast<int>(bulk.channels[i]), static_cast<int>(slow.channels[i]));
        CHECK_EQ(static_cast<int>(bulk.ccs[i]), static_cast<int>(slow.ccs[i]));
        CHECK_EQ(static_cast<int>(bulk.values[i]), static_cast<int>(slow.values[i]));
    }
}
