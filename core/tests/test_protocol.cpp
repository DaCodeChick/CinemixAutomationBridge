...
TEST_CASE("protocol: strip fader addressing (legacy table)") {
    ...
    CHECK_EQ(int(a.port), 1); CHECK_EQ(int(a.channel), 1); CHECK_EQ(int(a.cc), 0);
    a = p.stripFaderAddress(0, StripPath::Mix);
    CHECK_EQ(int(a.port), 1); CHECK_EQ(int(a.channel), 1); CHECK_EQ(int(a.cc), 2);
    a = p.stripFaderAddress(23, StripPath::Mix);
    CHECK_EQ(int(a.channel), 1); CHECK_EQ(int(a.cc), 94);
    // First HI strip (25): numbering restarts at CC 0 on channel 2, port 2.
    a = p.stripFaderAddress(24, StripPath::Chan);
    CHECK_EQ(int(a.port), 2); CHECK_EQ(int(a.channel), 2); CHECK_EQ(int(a.cc), 0);
    a = p.stripFaderAddress(35, StripPath::Mix);
    CHECK_EQ(int(a.port), 2); CHECK_EQ(int(a.channel), 2); CHECK_EQ(int(a.cc), 46);
    a = p.stripFaderFineAddress(35, StripPath::Mix);
    CHECK_EQ(int(a.cc), 47);
}
...
TEST_CASE("protocol: strip mute/touch addressing (legacy table)") {
    ...
    CHECK_EQ(int(a.port), 1); CHECK_EQ(int(a.channel), 3); CHECK_EQ(int(a.cc), 0);
    a = p.stripMuteAddress(23, StripPath::Mix);
    CHECK_EQ(int(a.channel), 3); CHECK_EQ(int(a.cc), 47);
    a = p.stripMuteAddress(24, StripPath::Chan);
    CHECK_EQ(int(a.port), 2); CHECK_EQ(int(a.channel), 4); CHECK_EQ(int(a.cc), 0);
    a = p.stripMuteAddress(35, StripPath::Mix);
    CHECK_EQ(int(a.channel), 4); CHECK_EQ(int(a.cc), 23);

    a = p.stripTouchAddress(0, StripPath::Chan);
    CHECK_EQ(int(a.port), 0); CHECK_EQ(int(a.channel), 3); CHECK_EQ(int(a.cc), 64);
    a = p.stripTouchAddress(23, StripPath::Mix);
    CHECK_EQ(int(a.channel), 3); CHECK_EQ(int(a.cc), 111);
    a = p.stripTouchAddress(24, StripPath::Chan);
    CHECK_EQ(int(a.channel), 4); CHECK_EQ(int(a.cc), 64);
    a = p.stripTouchAddress(35, StripPath::Mix);
    CHECK_EQ(int(a.channel), 4); CHECK_EQ(int(a.cc), 87);
}
...
TEST_CASE("protocol: master section addressing") {
    ...
    CHECK_EQ(int(a.port), 2); CHECK_EQ(int(a.channel), 5); CHECK_EQ(int(a.cc), 0);
    a = p.masterSelAddress();
    CHECK_EQ(int(a.port), 0); CHECK_EQ(int(a.channel), 5); CHECK_EQ(int(a.cc), 64);
    a = p.joyAxisAddress(0, 0);
    CHECK_EQ(int(a.port), 2); CHECK_EQ(int(a.channel), 2); CHECK_EQ(int(a.cc), 48);
    a = p.joyAxisAddress(0, 1);
    CHECK_EQ(int(a.cc), 50);
    a = p.joyAxisAddress(1, 0);
    CHECK_EQ(int(a.cc), 52);
    a = p.joyAxisAddress(1, 1);
    CHECK_EQ(int(a.cc), 54);
    a = p.joyMuteAddress(0);
    CHECK_EQ(int(a.channel), 4); CHECK_EQ(int(a.cc), 24);
    a = p.joyMuteAddress(1);
    CHECK_EQ(int(a.cc), 26);
    a = p.joySelAddress(0);
    CHECK_EQ(int(a.port), 0); CHECK_EQ(int(a.channel), 4); CHECK_EQ(int(a.cc), 88);
    a = p.joySelAddress(1);
    CHECK_EQ(int(a.cc), 90);
    a = p.auxMuteAddress();
    CHECK_EQ(int(a.port), 2); CHECK_EQ(int(a.channel), 5); CHECK_EQ(int(a.cc), 96);
}
...
TEST_CASE("protocol: remote control and value encoding") {
    ...
    CHECK_EQ(int(m.length), 3);
    CHECK_EQ(int(m.data[0] & 0x0F), 4); // channel 5
    CHECK_EQ(int(m.data[1]), 127);
    CHECK_EQ(int(m.data[2]), 127);
    CHECK_EQ(int(m.port), 0);
    m = p.remoteControl(false);
    CHECK_EQ(int(m.data[2]), 0);
    m = p.systemReset();
    CHECK(m.isSystemReset());
    CHECK_EQ(int(m.port), 0);

    CHECK_EQ(int(CinemixProtocol::muteByte(false)), 2);
    CHECK_EQ(int(CinemixProtocol::muteByte(true)), 3);
    for (uint8_t n = 0; n < 10; ++n) {
        CHECK_EQ(int(CinemixProtocol::auxMuteByte(n, false)), 2 * (n + 1));
        CHECK_EQ(int(CinemixProtocol::auxMuteByte(n, true)), 2 * (n + 1) + 1);
    }
}
...
TEST_CASE("protocol: fader quantization matches legacy truncation") {
    ...
    CHECK_EQ(int(msgs[0].data[2]), 127);

    msgs = p.encodeStripFader(0, StripPath::Chan, 0.0f);
    CHECK_EQ(int(msgs[0].data[2]), 0);

    // Legacy: int(value * 127) — truncation, so 0.5 → 63 (not 64).
    msgs = p.encodeStripFader(0, StripPath::Chan, 0.5f);
    CHECK_EQ(int(msgs[0].data[2]), 63);

    CHECK_EQ(int(quantize7(1.5f)), 127); // clamped
    CHECK_EQ(int(quantize7(-1.f)), 0);

    // Mute encoding: strip 5 Mix → ch3 CC 11, ON=3.
    MidiMessage m = p.encodeStripMute(5, StripPath::Mix, true);
    CHECK_EQ(int(m.data[0] & 0x0F), 2); // channel 3
    CHECK_EQ(int(m.data[1]), 11);
    CHECK_EQ(int(m.data[2]), 3);
    CHECK_EQ(int(m.port), 1);

    // Master fader → ch5 CC0 port2.
    m = p.encodeMasterFader(0.25f);
    CHECK_EQ(int(m.data[0] & 0x0F), 4);
    CHECK_EQ(int(m.data[1]), 0);
    CHECK_EQ(int(m.data[2]), 31); // 0.25*127 = 31.75 → 31
    CHECK_EQ(int(m.port), 2);
}
...
TEST_CASE("protocol: decode — faders (either CC of pair, legacy 7-bit)") {
    ...
    CHECK_EQ(int(ev.control.strip), 0);
    ...
    CHECK(ev.kind == EventKind::FaderPosition);
    CHECK_EQ(int(ev.control.strip), 1);
    CHECK(ev.control.path == StripPath::Chan);

    // HI side: strip 35 Mix = CC46/47 ch2.
    ev = p.decode(statusCh2, 46, 77);
    CHECK(ev.kind == EventKind::FaderPosition);
    CHECK_EQ(int(ev.control.strip), 35);
    CHECK(ev.control.path == StripPath::Mix);
...
TEST_CASE("protocol: decode — mutes, touch, SEL, joysticks, AUX") {
    ...
    CHECK_EQ(int(ev.control.index), 0);
    CHECK(ev.control.path == StripPath::Chan);
    ev = p.decode(0xB1, 54, 55); // joy 2 Y
    CHECK(ev.control.cls == ControlClass::JoyAxis);
    CHECK_EQ(int(ev.control.index), 1);
    CHECK(ev.control.path == StripPath::Mix);
    ...
    CHECK_EQ(int(ev.control.index), 2);
    CHECK(ev.on);
    ev = p.decode(0xB4, 96, 12);
    CHECK(!ev.on);
    CHECK_EQ(int(ev.control.index), 5);
    ...
    CHECK_EQ(int(ev.channel), 5);
    CHECK_EQ(int(ev.cc), 127);
...
TEST_CASE("protocol: fourteenBit profile encodes pair and combines on decode") {
    ...
    CHECK_EQ(msgs.size(), size_t(2));
    CHECK_EQ(int(msgs[0].data[1]), 0);
    CHECK_EQ(int(msgs[0].data[2]), 63);
    CHECK_EQ(int(msgs[1].data[1]), 1);
    CHECK_EQ(int(msgs[1].data[2]), 127);
