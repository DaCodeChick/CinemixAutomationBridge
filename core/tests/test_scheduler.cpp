...
TEST_CASE("scheduler: FIFO order for commands preserved") {
    ...
    CHECK_EQ(f.transport.sentToPort1.size(), size_t(3));
    CHECK_EQ(int(f.transport.sentToPort1[0].data[1]), 64);
    CHECK_EQ(int(f.transport.sentToPort1[1].data[1]), 65);
    CHECK_EQ(int(f.transport.sentToPort1[2].data[1]), 66);
    CHECK_EQ(f.sched.pending(), size_t(0));
}
...
TEST_CASE("scheduler: broadcast goes to both port streams") {
    ...
    CHECK_EQ(f.transport.sentToPort2.size(), size_t(1));
    CHECK_EQ(int(f.transport.sentToPort1[0].data[2]), 127);
    CHECK_EQ(int(f.transport.sentToPort2[0].data[2]), 127);
}
...
TEST_CASE("scheduler: position coalescing keeps only latest value") {
    ...
    CHECK_EQ(cc0, size_t(1));
    CHECK_EQ(int(f.transport.sentToPort1[0].data[2]), 30);
    CHECK_EQ(f.sched.coalescedCount(), size_t(2));
}
...
TEST_CASE("scheduler: identical position updates are both sent (dedupe is engine-level)") {
    ...
    f.sched.enqueuePosition(1, MidiMessage::controlChange(1, 2, 64, 1));
    f.sched.drainToEmpty();
    CHECK_EQ(f.transport.sentToPort1.size(), size_t(2));
...
TEST_CASE("scheduler: cancelPosition drops pending update") {
    ...
    f.sched.drainToEmpty();
    CHECK_EQ(f.transport.sentToPort1.size(), size_t(1));
    CHECK_EQ(int(f.transport.sentToPort1[0].data[1]), 2);
}
...
TEST_CASE("scheduler: high lane drains before main lane") {
    ...
    f.sched.enqueueCommand(cmd(3, 65, 1));
    f.sched.drainToEmpty();
    CHECK_EQ(int(f.transport.sentToPort1[0].data[1]), 70); // high first
    CHECK_EQ(int(f.transport.sentToPort1[1].data[1]), 64);
    CHECK_EQ(int(f.transport.sentToPort1[2].data[1]), 65);
}
...
TEST_CASE("scheduler: budget pacing caps the send rate") {
    ...
    for (int i = 0; i < 10; ++i)
        paced.enqueueCommand(cmd(3, uint8_t(64 + i), 1));

    // 2 messages/tick budget, no burst accumulation yet: tick 1 sends 2.
    CHECK_EQ(paced.tick() ? 1 : 0, 1);
    CHECK_EQ(f.transport.sentToPort1.size(), size_t(2));
    CHECK_EQ(paced.tick() ? 1 : 0, 1);
    CHECK_EQ(f.transport.sentToPort1.size(), size_t(4));
    // 6 remain.
    CHECK_EQ(paced.pending(), size_t(6));
    // Drain the rest unpaced for cleanup.
    paced.drainToEmpty();
    CHECK_EQ(paced.pending(), size_t(0));
}
...
TEST_CASE("scheduler: transport failure counts as dropped") {
    ...
    CHECK_EQ(f.sched.droppedTotal(), size_t(1));
}
